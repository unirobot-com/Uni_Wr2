...
class MotorControl:
    def __init__(self, encoder_pin1A, encoder_pin2B, pwm_pin1, pwm_pin2,
        ...
        self.encoder_actual_right = 1050  #实际测试得到的右轮编码器一圈数值
    ...
    def calculate_speed(self):
        ...
        round1 = delta_position1 / self.encoder_actual_left
        ...
        round2 = delta_position2 / self.encoder_actual_right
        ...
        self.left_wheel_odom = -self.encoder_position1 / self.encoder_actual_left * self.wheel_circumference
        self.right_wheel_odom = -self.encoder_position2 / self.encoder_actual_right * self.wheel_circumference
    ...
    def calculate_update2(self, enc_left, enc_right):
        ...
        if self.is_moving:
            ...
            dist_left = (-enc_left + self.encoder_last_1) / self.encoder_actual_left * self.wheel_diameter * math.pi
            dist_right =-((-enc_right+ self.encoder_last_2) / self.encoder_actual_right * self.wheel_diameter * math.pi)
            dist_turn_left = enc_left / self.encoder_actual_left * self.wheel_diameter * math.pi
            dist_turn_right = -enc_right / self.encoder_actual_right * self.wheel_diameter * math.pi
            # 计算小车移动的里程
            ...
            if self.teleop_keyboard_speed.angular.z > 0:
                theta = ((-dist_turn_right - dist_turn_left) / self.base_width) / 1.15 #1.09  #1.13
                ...
                theta = ((-dist_turn_right - dist_turn_left) / self.base_width) / 1.15 #1.18
                # theta = (-dist_turn_right / (self.base_width - 0.009) - dist_turn_left / self.base_width) / 1.13 #1.09  #1.13
            else:
                theta = ((-dist_turn_right - dist_turn_left) / self.base_width) / 1.13
            ...
            # print("x:", self.x, "y:", self.y, "theta_new:", self.theta_new)
